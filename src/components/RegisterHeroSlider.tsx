...
const RegisterHeroSlider: React.FC<RegisterHeroSliderProps> = ({ src, title, text, children }) => {
  return (
    <div className="relative !w-full h-[540px] sm:h-[360px] md:h-[481px] lg:h-[630px] box-border flex flex-col sm:flex-row gap-[8px] mx-0 px-0">
      ...
      {/* Δεξιά στήλη - 60% - Στρογγυλοποίηση αριστερών γωνιών */}
      <div className="relative w-full sm:w-[60%] h-[270px] sm:h-full flex items-center justify-center bg-[rgba(237,191,171,0.8)] p-4 rounded-tl-[8px] rounded-bl-[8px]">
        {children ? children : <p className="text-black">Εδώ θα έπρεπε να είναι η φόρμα</p>}
      </div>
    </div>
  );
};

export default RegisterHeroSlider;